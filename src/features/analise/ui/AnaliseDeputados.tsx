...
export default function AnaliseDeputados({ onFechar }) {
  ...
  const carregarDeputados = useCallback(async (params, append) => {
    ...
    try {
      const json = await api.get<{ dados: any[] }>('/deputados', Object.fromEntries(Object.entries(params).filter(([, v]: [string, unknown]) => v)) as Record<string, string>);
  ...
  const abrirDetalhe = useCallback(async (dep) => {
    ...
    if (!dadosCacheRef.current[id]) {
      ...
      try {
        const json = await api.get<any>(`/deputados/${id}/completo`);
