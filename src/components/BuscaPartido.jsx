...
export default function BuscaPartido({
  ...
  const fetchPartidos = useCallback(async () => {
    try {
      const resp = await fetch(`${API_BASE_URL}/busca/partidos`);
      if (!resp.ok) throw new Error('Erro ao carregar partidos');
      const json = await resp.json();
      setPartidos(json.opcoes || []);
    } catch (err) {
  ...
  const handleBuscar = useCallback(async () => {
    ...
    setError(null);
    try {
      const body = { partido_id: partidoId };
    ...
    setLoading(false);
  }, [partidoId, eleito, uf, onRPSearchComplete]);
    ...
    <div className="rp-section">
      ...
      <div className="rp-search-box">
        <div className="rp-search-row bfilter-row">
          ...
          <select value={partidoId} onChange={e => setPartidoId(e.target.value)} className="rp-search-input">
