...
function TabelaGen({ cabecalhos, linhas, maxWidthCol }) {
...
function SecaoOrcamento() {
  ...
  const carregar = useCallback(async () => {
    ...
    setErro(null);
    try {
      const resp = await fetch(`${API_BASE_URL}/senado/orcamento`);
...
function SecaoProcessos() {
  ...
  const buscar = useCallback(async (tipo, params, label) => {
    ...
    try {
      let url;
      if (tipo === 'processos') url = `${API_BASE_URL}/senado/processos`;
      else if (tipo === 'emendas') url = `${API_BASE_URL}/senado/processo/emendas`;
      else if (tipo === 'votacoes') url = `${API_BASE_URL}/senado/votacoes`;
      else return;
...
function SecaoAgenda() {
  ...
  const buscarAgendaDia = useCallback(async (params) => {
    ...
    try {
      const resp = await fetch(`${API_BASE_URL}/senado/agenda/dia/${data}`);
      const json = await resp.json();
      setResultados(prev => ({ ...prev, [subTabId]: { ...prev[subTabId], dados: json.dados || [], loading: false } }));
    } catch (err) {
      setResultados(prev => ({ ...prev, [subTabId]: { ...prev[subTabId], dados: [], loading: false, erro: err.message } }));
  ...
  const buscarAgendaMes = useCallback(async (params) => {
    ...
    try {
      const resp = await fetch(`${API_BASE_URL}/senado/agenda/mes/${data}`);
      const json = await resp.json();
      setResultados(prev => ({ ...prev, [subTabId]: { ...prev[subTabId], dados: json.dados || [], loading: false } }));
    } catch (err) {
      setResultados(prev => ({ ...prev, [subTabId]: { ...prev[subTabId], dados: [], loading: false, erro: err.message } }));
  ...
  const buscarEncontro = useCallback(async (params) => {
    ...
    try {
      const resp = await fetch(`${API_BASE_URL}/senado/encontro/${params.codigo}`);
      const json = await resp.json();
...
function SecaoComissoes() {
  ...
  const carregarComissoes = useCallback(async () => {
    ...
    setErro(null);
    try {
      const resp = await fetch(`${API_BASE_URL}/senado/comissoes`);
      const json = await resp.json();
  ...
  const abrirDetalheComissao = useCallback(async (com) => {
    ...
    if (!comDadosCacheRef.current[id]) {
      ...
      try {
        const resp = await fetch(`${API_BASE_URL}/senado/comissoes/${id}`);
        const json = await resp.json();
...
export default function AnaliseSenadores() {
  ...
  const carregarLista = useCallback(async () => {
    ...
    setListaErro(null);
    try {
      const resp = await fetch(`${API_BASE_URL}/senado/senadores`);
      const json = await resp.json();
  ...
  const abrirDetalhe = useCallback(async (sen) => {
    ...
    if (!dadosCacheRef.current[id]) {
      ...
      try {
        const resp = await fetch(`${API_BASE_URL}/senado/senadores/${id}/completo`);
        const json = await resp.json();
  ...
  const alterarSenatorSecao = useCallback((depId, secao) => {
    ...
    if (secao === 'emendas' && !emendasCache[depId]) {
      ...
      fetch(`${API_BASE_URL}/senado/processo/emendas?codigoParlamentarAutor=${depId}`)
        .then(r => r.json())
  ...
  const renderSenatorSecao = (senId, secao) => {
    ...
    if (!dados) return null;
    if (secao === 'geral') return <SecaoGeral senador={dados.senador} />;
